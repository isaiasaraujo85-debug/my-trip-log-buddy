...
export function CadastroTab() {
  ...
  const saveEditFunc = (id: string) => {
    ...
    setFuncionarios(funcionarios.map(f =>
      f.id === id ? { ...f, nome: editNome, matricula: editMatricula, funcao: editFuncao } : f
    ));
    setEditFuncId(null);
  };
  ...
  const saveEditVeic = (id: string) => {
    ...
    setVeiculos(veiculos.map(v =>
      v.id === id ? { ...v, modelo: editModelo, placa: editPlaca.toUpperCase() } : v
    ));
    setEditVeicId(null);
  };
        ...
        <CardContent className="space-y-4">
          ...
          {funcionarios.length > 0 && (
            <div className="space-y-3 pt-4 border-t">
              <h3 className="text-sm font-semibold text-muted-foreground">Funcionários Cadastrados</h3>
              {funcionarios.map((f) => (

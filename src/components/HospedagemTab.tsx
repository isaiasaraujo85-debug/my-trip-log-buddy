...
import { Plus, Trash2, BedDouble, Edit2, Save, X } from "lucide-react";
...
import { HospedagemRecord, Funcionario, EmpresaConfig, TipoHospedagem, AttachedImage } from "@/types";
import { FuncionarioSelect } from "./FuncionarioSelect";
import { DatePickerField } from "./DatePickerField";
import { ReportExportButton } from "./reports/ReportExportButton";
import { HospedagemReportContent } from "./reports/HospedagemReportContent";
...
export function HospedagemTab() {
  const [records, setRecords] = useLocalStorage<HospedagemRecord[]>("hospedagem-records", []);
  ...
  const startEdit = (record: HospedagemRecord) => {
    setEditId(record.id);
  ...
  const handleGeneratePdf = () => {
    generateHospedagemPdf(filteredRecords, dataInicio, dataFim, total, empresaConfig);
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label htmlFor="valor-hospedagem" className="text-xs">Valor (R$)</Label>
              <CurrencyInput id="valor-hospedagem" value={valor} onChange={setValor} />
            </div>
          </div>
          <ImageAttachButton images={imagens} onImagesChange={setImagens} label="Comprovante" />
          <ObservacaoField id="observacao-hospedagem" value={observacao} onChange={setObservacao} />
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center justify-between text-base">
            <span>Relatório de Hospedagem</span>
        ...
        {showReport && (
          <CardContent className="space-y-3">
            ...
            <ReportExportButton filename="relatorio-hospedagem" disabled={filteredRecords.length === 0} onGeneratePdf={handleGeneratePdf}>
              <HospedagemReportContent records={filteredRecords} dataInicio={dataInicio} dataFim={dataFim} total={total} empresaConfig={empresaConfig} />
            </ReportExportButton>
